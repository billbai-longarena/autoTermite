...
2.  **Run GUI (optional but recommended)**:
    ...
    ```
    *   On macOS, avoid Xcode's `/usr/bin/python3` (Tk 8.5) for Tkinter GUI.
        Prefer Python with Tk 8.6+ (e.g. Homebrew Python).
    *   The GUI can dynamically detect `codex` / `claude` processes and configure each process independently:
        *   `enable_agent_team`
        *   `automate_process`
        *   `new_chat_on_done`
    *   New observability panel (top bar + per-process runtime columns):
        *   daemon online/offline heartbeat
        *   loop/decision/injection/task counters
        *   per-process last trigger, cooldown/pause countdown, terminal stable time
    *   Batch controls:
...
1. 启动 GUI（进程识别 + 每进程配置 + 孤儿进程清理）：
   ```bash
   cd /Users/bill.bai/Desktop/autoTermite
   python3 termite_control_gui.py
   ```

2. 启动 daemon（读取 GUI 保存的配置并执行自动化）：
   ```bash
   cd /Users/bill.bai/Desktop/autoTermite
   sudo python3 termite_daemon.py
   ```
...
说明：
- `termite_control_gui.py` 不需要 `sudo`。
- `termite_daemon.py` 需要 `sudo`，因为要向目标 TTY 注入输入。
- GUI 中保存的配置写入 `termite_process_config.json`，daemon 会在轮询时自动加载。
- `start_termite.sh` 会先后台启动 daemon，再前台打开 GUI。
- daemon 标准输出会写入 `termite_daemon.stdout.log`。
...
*   `termite_runtime_status.json`: Daemon runtime snapshot consumed by GUI for status/trigger visibility.
